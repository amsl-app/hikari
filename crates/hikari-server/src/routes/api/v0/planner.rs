...
fn build_ical(
    entries: Vec<hikari_entity::planner_entry::Model>,
    milestones: Vec<hikari_entity::planner_milestone::Model>,
) -> String {
    ...
    let capacity = ascii_ical_len(entries.iter().map(|entry| entry.title.as_str()))
